---
name: git-workflow
description: "Enforces strict git workflow: branch naming conventions (feat/fix/hotfix/etc.), conventional commit messages, and explicit user authorization before any git command. Use whenever performing git operations (branching, committing, merging, pushing, reverting) in the user's repositories."
---

# Git Operations & Workflow

## 0. Hard Rules (User Preferences)

These rules override anything else in this skill:

1. **Never execute `git commit`.** Always deliver the commit message to the user as plain text inside a fenced code block, ready for them to copy/paste and run manually.
2. **Never execute `git push` unless the user explicitly asks for it** in the current turn. "Implement X" or "fix Y" does NOT imply push.
3. **Before starting any implementation work, verify the current branch:**
   - Run `git branch --show-current` (read-only, allowed).
   - If on `main`/`master`/`develop` or any branch not matching the task, create a new branch following the naming convention in section 1.
   - Ask the user for the branch type (`feat`, `fix`, etc.) if not obvious from the task.
   - Only after the correct branch exists, proceed with implementation.
